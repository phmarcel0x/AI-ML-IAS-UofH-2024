...
###### Simple Reaktive Behaviors ######    
def behaviour_approach_fish(ground_state):
    """ This function lets the robot to move towards the food resource. 
        This is an appetitve behaviour."""    
    # if the ground sensor are percieving a reading that is less than threshold means the robot is detecing the presence of the FOOD resource
    if ground_state[0] == 'fish' and ground_state[1] != 'fish':
        print("Turn to fish")
        turn_left()
    elif ground_state[1] == 'fish' and ground_state[0] != 'fish':
        print("Turn to fish")
        turn_right()


def behaviour_eat_fish(ground_state):
    """ This function is a consummatory behaviour for the robot if the robot is one the orange areas"""
    global food_level
    
    # If the both left and right ground state are percieving the fish resource, then the robot can consume this resource
    if ground_state[0] == 'fish' and ground_state[1] == 'fish':
            # the robot is on the resource, so the motor values are set to 0 as it has stopped moving to consume the resource
            stop()
            food_level += 0.5 # positive effect of eating the fish
            led_top.set(0x00ff00)
            print("Eat fish") # to confirm the behaviour is happening
        # This ensures that robot does not overcharge
            if food_level >= FOOD_LEVEL_UPPER_BOUND:
                    food_level = FOOD_LEVEL_UPPER_BOUND

def behaviour_turn_out_of_water(ground_state):
    """ This function lets the robot to move away from water. 
        This is an appetitve behaviour."""    
    # if the ground sensor are percieving a reading that is less than threshold means the robot is detecing the presence of the water
    if ground_state[0] != 'water' and ground_state[1] == 'water':
        print("Turn left from water")  
        turn_left()
    elif ground_state[1] != 'water' and ground_state[0] == 'water':
        print("Turn right from water")  
        turn_right()

    
def behaviour_swim_to_land(ground_state):
    """ This function is a behaviour to run away from dark areas"""   
    # If the both left and right ground states are water 
    if ground_state[0] == 'water' and ground_state[1] == 'water':
        forward_fast()
        print("Swimming fast") # to confirm the behaviour is happening
        print(distance_sensors_values)
        if distance_sensors_values[1] > THRESHOLD_WALL or distance_sensors_values[2] > THRESHOLD_WALL or distance_sensors_values[3] > THRESHOLD_WALL: 
            speed_left = 9
            speed_right = -9    
...
###### Motivation ######
def check_fish_stimulus(ground_state):
    """ This function is used for sensory input of an external stimuli for the fish resource."""
    fish_stimulus = None    
    return fish_stimulus

def check_dry_spot_stimulus(ground_state):
    """ This function is used for sensory input of an external stimuli for the fish resource."""
    dry_spot_stimulus = None  
    return dry_spot_stimulus

def motivation(ground_state):
    """ This is the main function for the motivational architecture of the robot."""
    global food_level, oxygen_level
    
    # Create the calculation of the motivation from the lecture 
    # Balance these (maybe here or in the behaviour coordination function.
    
    motivation_priority = None
    
 
    return motivation_priority 
